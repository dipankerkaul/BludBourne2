...
public abstract class GraphicsComponent extends ComponentSubject implements Component {
    ...
    protected Entity2.State _currentState;
    protected Entity2.Direction _currentDirection;
    ...
    protected GraphicsComponent(){
        ...
        _currentState = Entity2.State.WALKING;
        _currentDirection = Entity2.Direction.DOWN;
        _json = new Json();
    ...
    protected void updateAnimations(float delta){
        ...
        switch (_currentDirection) {
            case DOWN:
                if (_currentState == Entity2.State.WALKING) {
                    Animation<TextureRegion> animation = _animations.get(Entity2.AnimationType.WALK_DOWN);
                    if( animation == null ) return;
                    _currentFrame = animation.getKeyFrame(_frameTime);
                } else if(_currentState == Entity2.State.IDLE) {
                    Animation<TextureRegion> animation = _animations.get(Entity2.AnimationType.WALK_DOWN);
                    if( animation == null ) return;
                    _currentFrame = animation.getKeyFrames()[0];
                } else if(_currentState == Entity2.State.IMMOBILE) {
                    Animation<TextureRegion> animation = _animations.get(Entity2.AnimationType.IMMOBILE);
                    if( animation == null ) return;
                    _currentFrame = animation.getKeyFrame(_frameTime);
                }
                break;
            case LEFT:
                if (_currentState == Entity2.State.WALKING) {
                    Animation<TextureRegion> animation = _animations.get(Entity2.AnimationType.WALK_LEFT);
                    if( animation == null ) return;
                    _currentFrame = animation.getKeyFrame(_frameTime);
                } else if(_currentState == Entity2.State.IDLE) {
                    Animation<TextureRegion> animation = _animations.get(Entity2.AnimationType.WALK_LEFT);
            ...
            case UP:
                if (_currentState == Entity2.State.WALKING) {
                    Animation<TextureRegion> animation = _animations.get(Entity2.AnimationType.WALK_UP);
                    if( animation == null ) return;
                    _currentFrame = animation.getKeyFrame(_frameTime);
                } else if(_currentState == Entity2.State.IDLE) {
                    Animation<TextureRegion> animation = _animations.get(Entity2.AnimationType.WALK_UP);
            ...
            case RIGHT:
                if (_currentState == Entity2.State.WALKING) {
                    Animation<TextureRegion> animation = _animations.get(Entity2.AnimationType.WALK_RIGHT);
                    if( animation == null ) return;
                    _currentFrame = animation.getKeyFrame(_frameTime);
                } else if(_currentState == Entity2.State.IDLE) {
                    Animation<TextureRegion> animation = _animations.get(Entity2.AnimationType.WALK_RIGHT);
    ...
    protected Animation loadAnimation(String firstTexture, String secondTexture, Array<GridPoint2> points, float frameDuration){
        ...
        TextureRegion[][] texture1Frames = TextureRegion.split(texture1, Entity2.FRAME_WIDTH, Entity2.FRAME_HEIGHT);
        TextureRegion[][] texture2Frames = TextureRegion.split(texture2, Entity2.FRAME_WIDTH, Entity2.FRAME_HEIGHT);
    ...
    protected Animation loadAnimation(String textureName, Array<GridPoint2> points, float frameDuration){
        ...
        TextureRegion[][] textureFrames = TextureRegion.split(texture, Entity2.FRAME_WIDTH, Entity2.FRAME_HEIGHT);
    ...
    public Animation<TextureRegion> getAnimation(Entity2.AnimationType type){
